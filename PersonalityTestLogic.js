function calculateResults() {
  ...
  function getTraitMatchScore(traitScore, traitThreshold) {
    if (traitThreshold === "high") {
      return traitScore >= thresholds.high
        ? 3
        : traitScore >= thresholds.moderateHigh
        ? 2
        : traitScore >= thresholds.moderateLow
        ? 1
        : 0;
    } else if (traitThreshold === "moderate") {
      return traitScore >= thresholds.moderateLow &&
        traitScore <= thresholds.moderateHigh
        ? 2
        : 1;
    } else if (traitThreshold === "low") {
      return traitScore <= thresholds.low
        ? 3
        : traitScore <= thresholds.moderateLow
        ? 2
        : traitScore <= thresholds.moderateHigh
      ...
      return traitScore <= thresholds.low || traitScore >= thresholds.high
        ? 3
        : 1;
