...
export function usePersonalityTest() {
    ...
    const getThresholds = (minScore, maxScore) => ({
        low: minScore + (maxScore - minScore) * 0.25,   // Below 25% of total range
        high: minScore + (maxScore - minScore) * 0.75,  // Above 75% of total range
        moderateLow: minScore + (maxScore - minScore) * 0.25, // 25% of total range
        moderateHigh: minScore + (maxScore - minScore) * 0.75 // 75% of total range
    });
    ...
    const archetypes = [
        ...
        },
        {
            ...
            thresholds: {
                Openness: "moderate",
                Conscientiousiousness: "moderate",
                Extraversion: "high",
                Agreeableness: "high",
                Neuroticism: "low",
            },
    ...
    // This function determines how well a trait matches the archetype's threshold
    function getTraitMatchScore(traitScore, traitThreshold) {
        if (traitThreshold === "high") {
            return traitScore >= thresholds.high ? 1 : 0;  // High matches if the score is greater than or equal to the high threshold
        } else if (traitThreshold === "moderate") {
            return traitScore >= thresholds.moderateLow && traitScore <= thresholds.moderateHigh ? 1 : 0;  // Moderate matches within the range
        } else if (traitThreshold === "low") {
            return traitScore <= thresholds.low ? 1 : 0;  // Low matches if the score is lower than or equal to the low threshold
        } else if (traitThreshold === "lowOrHigh") {
            return traitScore <= thresholds.low || traitScore >= thresholds.high ? 1 : 0;  // Matches if low or high
        } else if (traitThreshold === "moderateToHigh") {
            return traitScore >= thresholds.moderateLow ? 1 : 0; // Moderate to high matches scores greater than or equal to moderateLow
        } else if (traitThreshold === "lowToModerate") {
            return traitScore <= thresholds.moderateHigh ? 1 : 0; // Low to moderate matches if score is below moderateHigh
        }
        return 0;
    }
